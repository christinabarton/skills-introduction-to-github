...
def index():
    ...
    # Select chart type
    if chart_type == "bar":
        fig = px.bar(df, x="Region", y="Export_Tons", color="Country", title="Bar Chart by Region")
    elif chart_type == "scatter":
        fig = px.scatter(df, x="Export_Value_USD", y="Export_Tons", color="Region", title="Scatter Plot")
    else:
        fig = px.box(df, x="Region", y="Export_Tons", color="Year", title="Box Plot by Year")
